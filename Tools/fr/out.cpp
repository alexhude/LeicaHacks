...
#define OUT_IMM_NOSHARP		0x1
#define OUT_IMM_SIGNED		0x2
...
static void out_imm(op_t &op, uint32_t flags = 0) {
    if ( (flags & OUT_IMM_NOSHARP) == 0 )
        out_symbol('#');

	if (flags & OUT_IMM_SIGNED)
...
void idaapi header(void) {
    ...
    if ( ash.header != NULL ) {
        for (const char **ptr = (const char **)ash.header; *ptr != NULL; ptr++) {
            MakeLine(*ptr,0);
        }
    }
}
    ...
    switch ( op.type ) {
        ...
        case o_imm:
            {
                ...
                else
				{
					...
					if (op_displ_imm_sign(op))
						out_imm(op, OUT_IMM_SIGNED);
					else
						out_imm(op);
				}
            }
        ...
        case o_displ:
            ...
            if ( op_displ_imm_r14(op) ) {
                ...
                out_symbol(',');
                OutChar(' ');
                out_imm(op, OUT_IMM_SIGNED | OUT_IMM_NOSHARP);
            }
            // @(R15, #i)
